...
export function acceptedToApplicant(m: MeetingBrief, joinUrl: string, passcode: string | null) {
  return {
    ...
    body: [
      ...
      ``,
      `[알아두실 것]`,
      `멘토의 이름과 회사는 끝까지 공개되지 않습니다. 다만 화상으로 만나므로`,
      `얼굴과 목소리는 드러날 수 있습니다. 카메라는 꺼진 채로 시작하고 켜는 것은`,
      `각자 고릅니다. 멘토의 신원을 알아내려 하거나 세션을 녹화·녹음하지 말아주세요.`,
    ]
      .filter((l) => l !== ``)
      .join("\n"),
  };
...
export function acceptedToMentor(m: MeetingBrief, hostUrl: string | null, joinUrl: string) {
  return {
    ...
    body: [
      ...
      ``,
      `[익명에 대해]`,
      `신청자에게는 ${m.mentorTitle} 로만 표시되고 이름·회사는 전달되지 않았습니다.`,
      `다만 화상으로 만나므로 얼굴과 목소리는 드러날 수 있습니다. 카메라는 꺼진 채로`,
      `시작하니 켤지 말지는 직접 정하시면 됩니다.`,
      `줌에 들어가신 뒤 표시 이름을 별명으로 바꿔주세요 —`,
      `참가자 목록에서 본인 이름 옆 [더 보기] → [이름 바꾸기] 입니다.`,
    ].join("\n"),
  };
}
