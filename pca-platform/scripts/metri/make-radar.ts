...
const COPY = {
  ...
  en: {
    ...
    card: (d: Deck) =>
      `Leading job area ${d.areas[0].value}, then ${d.areas[1].name} ${d.areas[1].value}. ` +
...
function radar(rows: Row[], key: string, hidden: boolean, c: (typeof COPY)[Lang]): string {
  ...
  const rings = [25, 50, 75, 100]
    .map((p) => `<polygon${p === 50 ? ' class="cm-rg-mid"' : ""} points="${j2(rows.map((_, i) => pt(i, n, p)))}"/>`)
    .join("");
  const spokes = rows
  ...
  const labs = rows
    .map((r, i) => {
      ...
      if (Math.abs(cs) < 0.28) { anchor = "middle"; y += sn < 0 ? -9 : 15; base = "auto"; }
  ...
  return (
    ...
    `<polygon class="cm-poly" points="${j2(rows.map((r, i) => pt(i, n, r.value)))}"/>` +
...
async function deck(attempt: number, lang: Lang): Promise<Deck> {
  ...
  return {
    ...
    areas: areas.map((a) => {
      ...
      return { name: a.name, value: Math.round(Number(a.scaled_score)) };
    }),
    top2: style.slice().sort((x, y) => y.value - x.value).slice(0, 2).map((r) => r.name),
  };
}

function measureHtml(d: Deck, lang: Lang): string {
  const c = COPY[lang];
  const bars = d.areas
...
<div class="cm">
  <div class="cm-in">
    ...
    <div class="cm-chart">
      ...
      <div class="cm-vals">
        <div class="cm-vlist" data-cm-vals="style">${vals(d.style)}</div>
        <div class="cm-vlist" data-cm-vals="axis" hidden>${vals(d.axis)}</div>
...
async function main() {
  for (const lang of ["ko", "en"] as Lang[]) {
    const d = await deck(ATTEMPT, lang);
    writeFileSync(join(process.cwd(), SITE[lang], "17-measure.html"), measureHtml(d, lang), "utf8");
    const [a, b] = await Promise.all(PAIR.map((n) => deck(n, lang)));
