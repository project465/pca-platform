...
전용 링크는 주소를 아는 사람이면 누구나 열 수 있다. 학교 홈페이지처럼 공개된
곳에 걸 때는 조건을 함께 준다.

| 조건 | 적는 법 | 뜻 |
|---|---|---|
| 학번 형태 | `9999999999` | 숫자 10자리만 |
| | `20########` → `209999999999` | 20 으로 시작하는 숫자 |
| | `A999999` | 영문 하나 + 숫자 6자리 |
| 이메일 도메인 | `hanyang.ac.kr` | 그 도메인만 |
| | `ac.kr` | 그 아래 도메인까지 (`hanyang.ac.kr` 통과) |
| | `hanyang.ac.kr, hyu.ac.kr` | 여럿은 쉼표로 |

마스크의 `9` 는 숫자, `A` 는 영문, `*` 는 숫자나 영문 하나다. 나머지 글자는
그대로 있어야 한다. **정규식을 그대로 받지 않는다** — 잘못 쓰면 아무도 못
들어오고, 어떤 식은 검사에 아주 오래 걸려 서버를 묶는다.

이메일 도메인을 걸면 등록 화면이 이메일을 함께 묻는다. 걸지 않으면 묻지 않는다.

조건은 링크마다 따로다. 학과용은 학번만, 일자리센터용은 이메일까지 받는 식으로
나눠 걸 수 있다.

**이미 돌고 있는 데이터베이스라면** 컬럼을 더해야 한다.

```sql
ALTER TABLE org_links ADD COLUMN IF NOT EXISTS login_id_mask TEXT;
ALTER TABLE org_links ADD COLUMN IF NOT EXISTS email_domains TEXT;
```
